...
export default function DesktopHero() {
  ...
  const { springX, springY } = useMouseParallax(shouldReduceAnimations ? 0 : 16);
  ...
  const layer1X = useTransform(springX, v => v * -0.6);
  const layer1Y = useTransform(springY, v => v * -0.6);
  const layer2X = useTransform(springX, v => v * 0.4);
  const layer2Y = useTransform(springY, v => v * 0.4);
      ...
      <div className="relative z-10 w-full mx-auto grid lg:grid-cols-2 items-center gap-12 lg:gap-16 px-6 md:px-16"
        ...
        <div className="flex flex-col">
          ...
          <motion.h1
            ...
            HARNESS<br />
            <span className="text-transparent" style={{ backgroundImage: 'linear-gradient(120deg, #e5a93c 0%, #f9bd22 38%, #00d2ff 100%)', backgroundClip: 'text', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              THE BOLT
            </span>
          </motion.h1>
          ...
          <motion.div
            ...
            <motion.div ref={ghostRef} style={{ x: ghostX, y: ghostY }} {...ghostHandlers}>
              <a href={waLink(messages.hero)} target="_blank" rel="noreferrer" onClick={() => trackWhatsApp('desktop-hero')}
                ...
                style={{ fontSize: 13, padding: '17px 38px', color: '#fff', background: 'linear-gradient(135deg, #25d366 0%, #128c7e 100%)', boxShadow: '0 4px 18px rgba(37,211,102,0.4)' }}>
